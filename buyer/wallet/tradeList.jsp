            	...
            	<div class="wallet walletDetail">
				    ...
				    <ul class="typeTilter">
				    	<li <c:if test="${settlementStatus==PENDING_SETTLE}">class="active"</c:if>>
				    		<a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${PENDING_SETTLE}&capitalFlowType=${capitalFlowType}">待结算</a></li>
				    	<li <c:if test="${settlementStatus==SETTLE_SUCCESS}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${SETTLE_SUCCESS}&capitalFlowType=${capitalFlowType}">结算成功</a></li>
				    	<li <c:if test="${settlementStatus==SETTLE_FAIL}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${SETTLE_FAIL}&capitalFlowType=${capitalFlowType}">结算失败</a></li>
				    	<li id="comeOut"><a href="#">筛选</a></li>
				    </ul>
				    <ul class="allType">
				    	<li <c:if test="${capitalFlowType==-1}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=-1">全部</a></li>
				    	<li <c:if test="${capitalFlowType==WITHDRAW_TO_WECHAT}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=${WITHDRAW_TO_WECHAT}">提现到微信</a></li>
				    	<li <c:if test="${capitalFlowType==REFUND}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=${REFUND}">退款</a></li>
				    	<li <c:if test="${capitalFlowType==RETURN}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=${RETURN}">退货</a></li>
				    	<c:if test="${user.role==2||user.role==4||user.role==6 }"><li <c:if test="${capitalFlowType==COMMISION}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=${COMMISION}">佣金</a></li></c:if>
				    	<li <c:if test="${capitalFlowType==WECHAT_CONSUME_GOODS}">class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/tradeList?settlementStatus=${settlementStatus}&capitalFlowType=${WECHAT_CONSUME_GOODS}">微信消费</a></li>
				    </ul>
				    <div class="detailDiv">
	...
	<script>
		...
		$(".back").tap(function(){
			if(!$(this).hasClass("prev")){
				window.location.href='<%=request.getContextPath()%>/buyer/wallet';
			}
		});
