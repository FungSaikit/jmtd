...
<div class="myOrder">
    ...
    <div class="allStatus">
    	<ul id="statusUl">
	    	<li <c:if test='${orderStatus==OBLIGATION }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${OBLIGATION}">待付款</a></li>
	    	<li <c:if test='${orderStatus==TO_BE_SHIPPED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${TO_BE_SHIPPED}">待发货</a></li>
	    	<li <c:if test='${orderStatus==TO_BE_SIGNED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${TO_BE_SIGNED}">待收货</a></li>
	    ...
	    <div class="childStatus">
	    	...
	    	<ul id="childStatusUl">
	    		<li <c:if test='${orderStatus==SIGNED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${SIGNED }">已签收</a></li>
	    		<li <c:if test='${orderServiceStatus==PENDING_SERVICE }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/serviceOrderManage?orderServiceStatus=${PENDING_SERVICE }">待服务</a></li>
	    		<li <c:if test='${orderServiceStatus==SERVICED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/serviceOrderManage?orderServiceStatus=${SERVICED }">已服务</a></li>
	    		<li <c:if test='${orderStatus==FINISHED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${FINISHED}">已完成</a></li>
	    		<li <c:if test='${orderStatus==CLOSED }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/orderManage?orderStatus=${CLOSED}">已关闭</a></li>
	    	</ul>
	    </div>
    </div>
    <div class="orderInfo">
...
<script>
	...
	$(function(){
		...
		function loadOrder(){
			var url="<%=request.getContextPath()%>/buyer/serviceOrderListAjax";
		...
		function loadSuccess(data){
			...
			for(var i=0;i<orderList.length;i++){
				...
				html+='<a href="<%=request.getContextPath()%>/product/shopProductList?shopId='+orderList[i].seller.userId+
				'"><span><i class="iconfont">&#xe606;</i>'+orderList[i].seller.shortCompanyName+'</span></a>';
				html+=((orderList[i].service.phoneNumber!=null)?'<i class="iconfont contactI phoneI" data-phone="'+orderList[i].service.phoneNumber+'">&#xe609;</i>':'');
				html+=((orderList[i].service.qqNumber!=null)?'<i class="iconfont contactI qqI" data-qqNumber="'+orderList[i].service.qqNumber+'">&#xe607;</i>':'');
				html+="</div>";
				...
				html+='<p class="allP">商品合计:<span>￥'+orderList[i].orderForm.orderMoney+'</span>(共<span>'+productNumber+'</span>件)　'+((orderList[i].orderForm.orderInstallMoney!=0)?'安装费合计:<span>￥'+orderList[i].orderForm.orderInstallMoney+'</span>':'')+'</p>';
