...
<script>
	...
	var offset=0;
	function loadOrder(){
		var url="<%=request.getContextPath()%>/buyer/orderSendUserDetailAjax";
		var params={offset:offset,logisticsCompanyName:"${logisticsCompanyName}",buyerId:"${buyerId}"};
	...
	function loadSuccess(data){
		...
		for(var i=0;i<ofList.length;i++){
			...
			for(var j=0;j<orderItemList.length;j++){
				...
				html+='	<p class="fl">下单时间：'+showDate(ofList[i].orderForm.payTime)+'</p>';
				...
				html+='<div>';
				html+=(orderItemList[j].orderItem.perpareSendNumber);
				html+='';
	...
	function loadSuccess(data){
		...
		for(var i=0;i<ofList.length;i++){
			...
			for(var j=0;j<orderItemList.length;j++){
				...
				html+='<div class="fr">数量：'+(orderItemList[j].orderItem.perpareSendNumber)+'</div></div>';
