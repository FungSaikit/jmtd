...
<script src="<%=request.getContextPath() %>/resources/common/jweixin-1.0.0.js"></script>
		...
		<script>
			var offset=0;
			function loadOrder(){
				var url="<%=request.getContextPath()%>/buyer/prePareReceiveFactoryAjax";
			...
			$(document).on("tap",".factoryList li",function(){
				window.location.href="<%=request.getContextPath()%>/buyer/prepareReceiveList?factoryName="+$(this).attr("data-str")
...
<script>
	...
	$(document).on("tap",".orderStock",function(){
		window.location.href="<%=request.getContextPath()%>/buyer/prepareReceiveUser?factoryName="+$(this).attr("data-str")
				+"&sellerId="+$(this).attr("data-id");
	});
	/* var canLoad=true;
	var noDate=false;
