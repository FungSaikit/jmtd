...
<head>
	...
	<style>
		...
		.orderDetail .goodsInfo .shopOrder .commodity, .myOrder .orderInfo .shopOrder .commodity {
...
<div class="myOrder">
    <div class="title"><i class="iconfont prevPage" >&#xe63f;</i>售后服务</div>
    <div class="searchDiv">
    	<div>
    ...
    <div class="allStatus">
    	<ul id="statusUl">
	    	<li <c:if test='${status==0 }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/afterSaleList?status=0">待处理</a></li>
	    	<li <c:if test='${status==1 }'>class="active"</c:if>><a href="<%=request.getContextPath()%>/buyer/afterSaleList?status=1">已处理</a></li>
	    </ul>
	</div>
    <div class="orderInfo">
    </div>
...
<script>
	...
	$(function(){
		...
		function loadOrder(){
			var url="<%=request.getContextPath()%>/buyer/afterSaleListAjax";
		...
		function loadSuccess(data){
			...
			for(var i=0;i<afterSaleModel.length;i++){
			     ...
			     html+='</a><div class="commDiv"><p><a href="">'+afterSaleModel[i].product.productCode+'</a></p></div>';
			     html+='</div>';
			     html+='<div class="btnGroup"><button class="detailBtn">查看详情</button></div></div>';
			}
