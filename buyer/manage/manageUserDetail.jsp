...
<div class="distributor">
    ...
    <form class="form"  onsubmit="return checkForm();">
    	<input type="hidden" name="applyId" value="${application.applyId }"/>
    	<input type="hidden" name="userId" value="${application.userId }"/>
        <c:if test="${user.role==AGENCY }">
		...
		<c:if test="${user.checkStatus==AUTHORIZING }">
	        <br/>
	        <input type="button" class="submit" onclick="passApplication()" value="通过审核"/>
	        <br/>
	        <input type="button" class="submit" onclick="dispassApplication()" value="不通过审核"/>
        </c:if>
    </form>
</div>
<div class="tip"></div>
...
<script>
	function passApplication(){
		$("form").attr("action","<%=request.getContextPath()%>/buyer/passApplication");
		$("form").submit();
	}
	
	function dispassApplication(){
		$("form").attr("action","<%=request.getContextPath()%>/buyer/dispassApplication");
		$("form").submit();
	}
