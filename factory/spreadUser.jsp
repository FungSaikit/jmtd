...
<head>
    ...
    <link href="<%=request.getContextPath() %>/resources/factory/assets/css/classify.css?88" rel="stylesheet">
...
<script>
	...
	function loadList(){
		var url;
		url="<%=request.getContextPath()%>/factory/spreadUserListMore?offset="+offset+
				"&keyword=${keyword}";
	...
	function success(data){
		...
		for(var i=0;i<list.length;i++){
			...
			//html+='<span class="memPhone">'+((list[i].clientCellphoneNumber==null)?'':list[i].clientCellphoneNumber)+'</span>';
			html+='<span class="memPhone"></span>';
			html+='</div><span class="toSpan"><i class="iconfont goToDetail">&#xe624;</i></span>';
	...
	function getClientInfo(id){
		var url="<%=request.getContextPath()%>/factory/getClientInfo";
	...
	$(".showHide").tap(function(){
		if($(this).hasClass("show")){
			//显示被删除的客户
			//进行页面跳转，以下三行几乎没用
			window.location.href='<%=request.getContextPath()%>/factory/spreadUserList?show=1';
			$(this).find("i").html("&#xe62a;");
			...
			//隐藏被删除的客户
			//进行页面跳转，以下三行几乎没用
			window.location.href='<%=request.getContextPath()%>/factory/spreadUserList?show=0';
			$(this).find("i").html("&#xe62b;");
			$(this).addClass("show");
