...
<div class="orderStock">
    ...
    <div class="allStatus">
    	<ul id="statusUl">
	    	<li <c:if test="${orderStockStatus==1 }">class="active"</c:if>><a href="<%=request.getContextPath()%>/factory/newOrderStation?orderStockStatus=1">待处理</a></li>
	    	<li <c:if test="${orderStockStatus==2 }">class="active"</c:if>><a href="<%=request.getContextPath()%>/factory/newOrderStation?orderStockStatus=2">待发货</a></li>
	    	<li <c:if test="${orderStockStatus==3 }">class="active"</c:if>><a href="<%=request.getContextPath()%>/factory/newOrderStation?orderStockStatus=3">已发货</a></li>
	    </ul>
    </div>
    <div class="stockInfo">
    	<c:forEach items="${ofmList}" var="orderForm">
   		<div class="orderStock">
   			<div class="orderStockHead">
   				<p>下单时间：<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${orderForm.orderForm.payTime}"/><br>
   					编号：${orderForm.buyer.userIdenticalCode }</p>
   			</div>
   			<div class="orderStockBody">
   				<c:forEach items="${orderForm.orderItemList }" var="orderItem">
   				<div class="stock" data-id="${orderItem.orderItem.orderItemId }">
   					<div class="order">
   						<div class="fl">型号：${orderItem.product.productCode }</div>
   						...
   						<div>
	   						...
	   						<div>
	   							...
	   							<c:if test="${orderStockStatus==2}">
	   								${orderItem.orderItem.spotGoodsNum}
	   							</c:if>
	   						</div>
	   						<span>个</span>
	   						</c:if>
	   						<c:if test="${orderStockStatus==3}">
	   							<label>已发${empty orderItem.orderItem.factorySendNumber?orderItem.orderItem.receiveNumber:orderItem.orderItem.factorySendNumber}个</label>
	   						</c:if>
	   					</div>
	   					<div class="imgBox">
   					...
   					<div class="stockNum">
   						<c:if test="${orderStockStatus==1||orderStockStatus==2 }">
		   					<label>急单：${!empty orderItem.orderItem.isRushOrder?'急单':'否'}</label>
		   					<button class="submitOrderItem" data-id="${orderItem.orderItem.orderItemId }">提交</button>
   						</c:if>
	   				</div>
   				</div>
    ...
    <form action="<%=request.getContextPath()%>/factory/<c:if test="${orderStockStatus==1 }">inputOrderStation</c:if><c:if test="${orderStockStatus==2}">factorySendStock</c:if>">
