...
<div class="distributor">
    ...
    <form class="form" <c:choose><c:when test="${application.checkStatus==AUTHENTICATION_SUCCESS||application.isUpdate==1 }">action="<%=request.getContextPath()%>/factory/updateAuthentication"</c:when><c:otherwise>action="<%=request.getContextPath()%>/factory/authentication"</c:otherwise></c:choose> onsubmit="return checkForm();">
        ...
        <div class="imgDiv">
             <div class="imgContent notEmpty">
                 <input type="hidden" class="imgUrl" value="${application.logoPic}" name="logoPic"/>
                 <img class="smallImg" src="#" data-src="<c:if test='${!empty application }'>${application.logoPic}</c:if>" alt="" <c:if test='${empty application||empty application.logoPic }'>style="display: none;"</c:if>/>
                 <span class="add" <c:if test='${!empty application&&!empty application.logoPic }'>style="display: none;"</c:if>><i class="iconfont">&#xe628;</i></span>
                 <span class="cancel" <c:if test='${!(application.checkStatus==AUTHENTICATION_SUCCESS&&!empty application.logoPic||application.isUpdate==1)}'>style="display: none;"</c:if>><i class="iconfont">&#xe632;</i></span>
             </div>
             <div class="imgTitle">厂家logo</div>
         </div>
         <div class="imgDiv">
             <div class="imgContent notEmpty">
...
<c:choose>
	<c:when test="${application.checkStatus==AUTHENTICATION_SUCCESS }">
		<jsp:include page="/jsp/factory/footer.jsp"></jsp:include>
	</c:when>
	<c:otherwise><jsp:include page="/jsp/buyer/footer.jsp"></jsp:include></c:otherwise>
</c:choose>
