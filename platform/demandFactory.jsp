...
<body>
        ...
        <div class="nav-tabs">
        	<button class="btn <c:if test='${navActive==UNAUTHORIZED }'>navActive</c:if>" onclick="window.location.href='<%=request.getContextPath() %>/platform/manageDemandFactory?checkStatus=${UNAUTHORIZED}'">未认证</button>
            <button class="btn <c:if test='${navActive==AUTHORIZING }'>navActive</c:if>" onclick="window.location.href='<%=request.getContextPath() %>/platform/manageDemandFactory?checkStatus=${AUTHORIZING}'">待审核</button>
            <button class="btn <c:if test='${navActive==AUTHENTICATION_SUCCESS }'>navActive</c:if>" onclick="window.location.href='<%=request.getContextPath() %>/platform/manageDemandFactory?checkStatus=${AUTHENTICATION_SUCCESS }'">审核通过</button>
            <button class="btn <c:if test='${navActive==AUTHENTICATION_FAILURE }'>navActive</c:if>" onclick="window.location.href='<%=request.getContextPath() %>/platform/manageDemandFactory?checkStatus=${AUTHENTICATION_FAILURE }'">审核不通过</button>
        	<button class="btn <c:if test='${navActive==UPDATE_AUTHORIZING }'>navActive</c:if>" onclick="window.location.href='<%=request.getContextPath() %>/platform/manageDemandFactory?checkStatus=${UPDATE_AUTHORIZING }'">信息修改待审核</button>
        </div>
        ...
        <!-- [[分页-->
        <div class="paging">
            <span class="prev btn btn-primary" onclick="window.location.href='<%=request.getContextPath()%>/platform/manageDemandFactory?checkStatus=${navActive}&offset=${userList.before}&keyword=${keyword }'">上一页</span>
            ...
            <span class="next btn btn-primary" onclick="window.location.href='<%=request.getContextPath()%>/platform/manageDemandFactory?checkStatus=${navActive}&offset=${userList.after}&keyword=${keyword }'">下一页</span>
        </div>
        <!-- 分页]]-->
	...
	<script>
		function searchByKeyword(){
			window.location.href="<%=request.getContextPath()%>/platform/manageDemandFactory?checkStatus=${navActive}&keyword="+$("#keyword").val();
		}
	</script>
</body>
</html>