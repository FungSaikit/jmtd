    ...
    <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <c:forEach items="${capitalFlowList}" var="cf" varStatus="i">
                    	<tr>
                        	<td><a href="<%=request.getContextPath()%>/platform/seeOrderDetail?capitalFlowId=${cf.capitalFlow.capitalFlowId}">${cf.capitalFlow.capitalFlowId }</a></td>
                        	<td>
                        		<c:choose>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WECHAT_CONSUME_GOODS}">微信购买商品</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==GOODS_MONEY}">货款</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==COMMISION}">佣金</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WITHDRAW_TO_WECHAT}">提现到微信零钱</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WITHDRAW_TO_ACCOUNT}">提现到银行卡</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WECHAT_ONLINE_GOODS}">微信上线商品</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WECHAT_LAPM_RENT}">微信租赁灯位</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==RETURN}">退货</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==REFUND}">退款</c:when>	
		                    	</c:choose>
                        	</td>
                        	<td>
	                           	<c:choose>
		                    		<c:when test="${cf.capitalFlow.settlementStatus==PENDING_SETTLE}">待结算</c:when>
		                    		<c:when test="${cf.capitalFlow.settlementStatus==SETTLE_SUCCESS}">结算成功</c:when>
		                    		<c:when test="${cf.capitalFlow.settlementStatus==SETTLE_FAIL}">结算失败</c:when>
		                    	</c:choose>
	                    	</td>
                           <td class="textRight moneyText" <c:choose><c:when test="${cf.capitalFlow.capitalFlowType==WITHDRAW_TO_ACCOUNT}">style="color:green"</c:when></c:choose>><c:choose>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==GOODS_MONEY}">+</c:when>
		                    		<c:when test="${cf.capitalFlow.capitalFlowType==WITHDRAW_TO_ACCOUNT}">-</c:when>
		                    	</c:choose>${cf.capitalFlow.money }</td>
                           <td>${cf.capitalFlow.creatTime}</td>
                           <td>${cf.sendTime}</td>
                           <td>${cf.capitalFlow.settleTime }</td>
	                   	</tr> 
                    </c:forEach> 
                    </tbody>
                </table>
